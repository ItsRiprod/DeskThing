...
const Boards: React.FC<defaultProps> = ({ handleSendGet, data }) => {
  const sortBoards = (boards: any[]) => {
    return boards.sort((a, b) => {
      ...
      // If both are starred or both are not starred, sort by closed status
  ...
  const [boards, setBoards] = useState<any[]>(() => sortBoards(data));
  ...
  const handleClick = (id: string) => {
    handleSendGet('get', 'lists_from_board', { id: id });
  };
      ...
      {boards.map((board: any) => (
        ...
        >
          ...
          <div className="board_title">
            <h1>{board.name}</h1>
            <p>{' Starred: ' + board.starred}</p>
          </div>
          <div className="board_starred">
            <svg
              width="100px"
              height="100px"
              viewBox="0 0 48 48"
            ...
            >
              <path
                fillRule={board.starred ? 'inherit' : 'evenodd'}
                clipRule="evenodd"
                ...
                fill={board.starred ? 'gold' : '#333333'}
              />
            </svg>
          </div>
        </button>
