    ...
    switch (event) {
      ...
      case "data":
        ...
        if (args[0]?.settings) {
          ...
          sendDataToMainFn("add", settings2);
        }
        break;
      case "get":
        handleGet(...args);
...
var isDataOutOfDate = (timestamp) => {
  const currentTime = (/* @__PURE__ */ new Date()).getTime();
  const dataTime = new Date(timestamp).getTime();
  const hoursDifference = (currentTime - dataTime) / (1e3 * 60 * 60);
  return hoursDifference;
};
var getCurrentWeather = async () => {
  try {
...
var getCityWeather = async (loc_key) => {
  try {
