...
type Component = URIType | string | number | undefined;

export function encodeQueryString(values: Record<string, any>): string {
  let str = '?';
 ...
 */
export function getComponents(uri: TypedURI, format: URIFormat): Component[] {
  ...
  switch (uri.type) {
    ...
    case URITypeMap.TOPLIST:
      return [URIParticle.TOP, uri.toplist].concat(
        uri.global || !uri.country
          ? [URIParticle.GLOBAL]
          : ['country', uri.country]
      );
    ...
    case URITypeMap.PROFILE:
      if (uri.args && uri.args.length > 0) {
        return [URIParticle.USER, encodeComponent(uri.username, format)].concat(
          uri.args
        );
      }
