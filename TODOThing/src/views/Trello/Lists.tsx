...
const Lists: React.FC<ListsProps> = ({ handleSendGet, data }) => {
  ...
  useEffect(() => {
    setStarred('662f56f299216978d05e3e1e'); // This is hardcoded - i'll fix it later
  }, [])
    ...
    <div className="list_container">
      {data.map((list: any) => (
        <div key={list.id} className="list_item">
          <button onClick={() => handleSendGet('cards_from_list', list.id)} className="list_content">
            <h1>{list.name}</h1>
          </button>
          <div className="board_starred">
            ...
            >
              <path
                fillRule={list.id == starred ? 'inherit' : 'evenodd'}
                clipRule="evenodd"
                ...
                fill={list.id == starred ? 'gold' : '#333333'}
              />
            </svg>
          </div>
        </div>
