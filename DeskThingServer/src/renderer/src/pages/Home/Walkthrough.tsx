...
import { IconCarThing, IconLogoGearLoading, IconRefresh } from '../../assets/icons'
...
const Walkthrough = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const listener = ClientStore.on('ADBDevices', (devices) =>
      handleConnectedDevices(devices as string[])
    )
    const otherListener = ClientStore.on('Connection', (connection) =>
      setConnections(connection as number)
    )
