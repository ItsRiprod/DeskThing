import { IconLayoutgrid } from '@renderer/assets/icons/index.ts'
import Button from '@renderer/components/Button.tsx'
import useMappingStore from '@renderer/stores/mappingStore.ts'
import { ActionReference, ButtonMapping, EventMode, Key } from '@shared/types/index.ts'
...
const AvailableKeys: React.FC<AvailableKeysProps> = ({
  ...
  return (
    <div className="absolute shrink-0 h-full w-fit right-0 md:relative">
      <div
        className={`${
          actionsExpanded ? 'w-48 ' : 'w-0'
        } transition-[width] md:w-64 border-l border-gray-700 relative h-full`}
      >
        <div
          className={`${
            actionsExpanded ? 'overflow-y-auto px-2' : 'overflow-hidden'
          } transition-[padding] md:px-2 w-full overflow-x-hidden md:overflow-y-auto relative bg-zinc-950 h-full`}
        >
