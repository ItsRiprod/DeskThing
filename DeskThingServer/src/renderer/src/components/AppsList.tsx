...
const AppsList = (): JSX.Element => {
  ...
  const handleDetails = (appIndex: number): JSX.Element => {
    return <DisplayAppData appIndex={appIndex} data={appsList} />
  }
        ...
        <div className="pt-5 w-[100%] flex 2xl:flex-row 2xl:flex-wrap flex-col items-center gap-2">
          ...
          {(appsList.apps as App[]).map((app, appIndex) => (
            ...
            >
              ...
              <button onClick={() => handleDetails(app.name)}>Details</button>
              {app.enabled ? (
                <div className="flex items-baseline md:flex-row flex-col">
                  ...
                  <button
                    ...
                    onClick={() => handleDisableApp(appIndex)}
                  >
                    <IconPause />
                  </button>
                  <button
