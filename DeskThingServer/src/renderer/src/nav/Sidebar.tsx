...
const Sidebar: React.FC<SidebarProps> = ({ children, className }) => {
  ...
  return (
    <div
      className={`${sidebarVisible ? 'w-40 max-w-40' : 'max-w-0 w-0'} inset-0 xs:max-w-48 transition-[width,max-width,min-width] z-10 min-w-0 md:min-w-48 md:max-w-48 xs:w-24 flex-shrink-0 xs:relative h-full border-r absolute border-zinc-800 bg-zinc-950 ${className}`}
    >
      <nav
