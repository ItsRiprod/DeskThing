...
export const isValidActionReference = (action: ActionReference): boolean => {
  if (typeof action !== 'object') {
    Logger.log(MESSAGE_TYPES.ERROR, `validateActionReference: action is not a valid object`)
    return false
  }
  if (typeof action.id !== 'string') {
    Logger.log(MESSAGE_TYPES.ERROR, `validateActionReference: id is not a valid string`)
    return false
  }
  if (typeof action.source !== 'string') {
    Logger.log(MESSAGE_TYPES.ERROR, `validateActionReference: source is not a valid string`)
    return false
  }
