...
function decodeQueryString(str: string): QueryParams {
  return str.split('&').reduce((object: QueryParams, pair) => {
 ...
 * @private
 */
function decodeComponent(component: string, format?: URIFormat): string {
  const part =
    format === URIFormat.URI ? component.replace(/\+/g, '%20') : component;
 ...
 */
export function splitIntoComponents(strURI: string): SplittedURI {
  let components;
  let format;
  let query;
  let anchor;
 ...
 * @private
 */
export function parseFromComponents(
  components: string[],
  format: URIFormat,
  query?: QueryParams
): URI {
  ...
  const _getNextComponent = (): string => {
  ...
  const _getBase62IdComponent = (): string => {
    const component = _getNextComponent();
  ...
  const _getRemainingComponents = (): string[] => {
    return components.slice(_current);
  };

  const _getRemainingString = (): string => {
  ...
  switch (part) {
    ...
    case URITypeMap.ALBUM:
      return factory.albumURI(
        _getBase62IdComponent(),
        parseInt(_getNextComponent(), 10)
      );
    case URITypeMap.AD:
    ...
    case URITypeMap.TEMP_PLAYLIST:
      return factory.temporaryPlaylistURI(
        _getNextComponent(),
        _getRemainingString()
      );
    ...
    case URITypeMap.TRACKSET: {
      ...
      let tracksArray: string | string[] = _getNextComponent();
      const hashSign = _getNextComponent();
      let index: number | null = parseInt(_getNextComponent(), 10);
    ...
    case URITypeMap.LOCAL_TRACK: {
      ...
      const artistName =
        artistNameComponent && decodeComponent(artistNameComponent, format);
      const albumNameComponent = _getNextComponent();
      const albumName =
        albumNameComponent && decodeComponent(albumNameComponent, format);
      const trackNameComponent = _getNextComponent();
      const trackName =
        trackNameComponent && decodeComponent(trackNameComponent, format);
